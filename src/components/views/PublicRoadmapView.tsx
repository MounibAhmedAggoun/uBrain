...
export function PublicRoadmapView({ slug }: { slug: string }) {
  ...
  useEffect(() => { if (!supabase) return; supabase.auth.getSession().then(({ data }) => setSignedIn(Boolean(data.session))); const listener = supabase.auth.onAuthStateChange((_event, session) => setSignedIn(Boolean(session))); return () => listener.data.subscription.unsubscribe() }, [])
  ...
  const submitComment = async () => { if (!comment.trim() || !supabase || !signedIn) return; try { await addRoadmapComment(roadmap.id, undefined, comment.trim()); setComments(await loadPublishedComments(roadmap.id)); setComment('') } catch { setNotice('Unable to post comment. Please try again.') } }
